...
```{r}
df <- data.frame(year, lifeExp, gapminder$country, gapminder$continent)
names(df)[3]<-"country"
names(df)[4]<-"continent"
data.frame(df, stringsAsFactors = TRUE)
year <- c(gapminder$year)
lifeExp <-c(gapminder$lifeExp)
country <-(c(gapminder$country))
continent <-c(gapminder$continent)
is.numeric(country)
is.factor(country)
...
```{r}
select(gapminder, country:lifeExp, -lifeExp)
```
...
```{r}
gapminder %>%
     mutate(tibble(gdpBill = gdpPercap*pop)) 
```
...
```{r}
select(gapminder, country:lifeExp, -lifeExp) %>%
  mutate(CorrectLife = ifelse(lifeExp == 68, 70, lifeExp))
```
